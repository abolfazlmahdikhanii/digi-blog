    ...
    <div className="w-11/12 mx-auto px-4">
      <div className="flex justify-center border-b mb-8">
        <div className="flex items-center space-x-8 text-sm text-muted-foreground">
          <Link
            href="#"
            className="text-foreground font-semibold border-b-2 border-foreground pb-3"
          >
            For you
          </Link>
...
export async function getStaticProps() {
  ...
  try {
    const posts = await postModel
      .find({})
      .populate("category") // Refers to 'Category' model
      .populate("author", "name username") // Refers to 'User' model
      .lean();
    ...
    return {
      ...
      revalidate: 60 * 60 * 2, // Optional: ISR revalidation
    };
  } catch (error) {
